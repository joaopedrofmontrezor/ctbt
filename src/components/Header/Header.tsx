    ...
    <header className={`${styles.header} ${isScrolled ? styles.scrolled : ''}`}>
      <div className={`container ${styles.container}`}>
        ...
        <nav className={`${styles.nav} ${isMenuOpen ? styles.navOpen : ''}`}>
          ...
          <a href="#arena" onClick={() => setIsMenuOpen(false)}>A Arena</a>
          <a href="#professores" onClick={() => setIsMenuOpen(false)}>Professores</a>
          <a href="#bar" onClick={() => setIsMenuOpen(false)}>Bar & Gastronomia</a>
          <a href="#localizacao" onClick={() => setIsMenuOpen(false)}>Localização</a>
        </nav>

        <div className={styles.actions}>
          <a href="https://wa.me/5516997168587" target="_blank" rel="noreferrer" className="btn-primary">
            Agendar
          </a>
          <button 
